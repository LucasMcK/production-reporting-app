...
const SearchBar = ({
    ...
    const [yearPlaceholderText, setYearPlaceholderText] = useState(yearPlaceholder);
    const [monthPlaceholderText, setMonthPlaceholderText] = useState(monthPlaceholder);
    const [locationPlaceholderText, setLocationPlaceholderText] = useState(locationPlaceholder);
    ...
    const handleMonthKeyDown = (e) => {
        if ((e.key === 'Backspace' || e.key === 'Delete') && monthValue.length === 0) {
          e.preventDefault();
    ...
    const handleLocationKeyDown = (e) => {
        if ((e.key === 'Backspace' || e.key === 'Delete') && searchValue.length === 0) {
          e.preventDefault();
    ...
    const onLocationBlur = () => setLocationPlaceholderText(locationPlaceholder);
    ...
    return (
        <div className="search-bar-container">
            ...
            />
            <span className="default-text">.xls</span>
        </div>
      );
};
